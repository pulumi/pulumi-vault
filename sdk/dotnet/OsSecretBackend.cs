// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Vault
{
    /// <summary>
    /// Manages OS Secrets Engine backend configuration in a Vault server. The OS Secrets Engine manages credentials
    /// for operating system accounts on remote hosts via SSH. This resource requires Vault 2.0.0 or later.
    /// 
    /// The OS Secrets Engine mount itself is managed separately, typically with `vault.Mount`. This resource only manages
    /// backend configuration for an existing OS mount.
    /// 
    /// Before mounting the OS Secrets Engine, the external OS plugin must already be registered in Vault's plugin catalog.
    /// You can register it with the `vault.Plugin` resource.
    /// 
    /// The examples below use the canonical plugin name `vault-plugin-secrets-os`. If your Vault cluster registers the
    /// OS plugin under a different catalog name, use that name in `vault_mount.type` instead.
    /// 
    /// See the [Vault documentation](https://www.vaultproject.io/docs/secrets/os) for more information.
    /// 
    /// ## Example Usage
    /// 
    /// ### Register Plugin And Configure Backend
    /// 
    /// ```csharp
    /// using System.Collections.Generic;
    /// using System.Linq;
    /// using Pulumi;
    /// using Vault = Pulumi.Vault;
    /// 
    /// return await Deployment.RunAsync(() =&gt; 
    /// {
    ///     var os = new Vault.Plugin("os", new()
    ///     {
    ///         Type = "secret",
    ///         Name = "vault-plugin-secrets-os",
    ///         Version = "v0.1.0+ent",
    ///     });
    /// 
    ///     var osMount = new Vault.Mount("os", new()
    ///     {
    ///         Path = "os",
    ///         Type = os.Name,
    ///     });
    /// 
    ///     var osOsSecretBackend = new Vault.OsSecretBackend("os", new()
    ///     {
    ///         Mount = osMount.Path,
    ///     });
    /// 
    /// });
    /// ```
    /// 
    /// ### Basic Configuration
    /// 
    /// ```csharp
    /// using System.Collections.Generic;
    /// using System.Linq;
    /// using Pulumi;
    /// using Vault = Pulumi.Vault;
    /// 
    /// return await Deployment.RunAsync(() =&gt; 
    /// {
    ///     var os = new Vault.Mount("os", new()
    ///     {
    ///         Path = "os",
    ///         Type = "vault-plugin-secrets-os",
    ///     });
    /// 
    ///     var osOsSecretBackend = new Vault.OsSecretBackend("os", new()
    ///     {
    ///         Mount = os.Path,
    ///     });
    /// 
    /// });
    /// ```
    /// 
    /// ### Advanced Configuration
    /// 
    /// ```csharp
    /// using System.Collections.Generic;
    /// using System.Linq;
    /// using Pulumi;
    /// using Vault = Pulumi.Vault;
    /// 
    /// return await Deployment.RunAsync(() =&gt; 
    /// {
    ///     var os = new Vault.Mount("os", new()
    ///     {
    ///         Path = "os-prod",
    ///         Type = "vault-plugin-secrets-os",
    ///     });
    /// 
    ///     var osOsSecretBackend = new Vault.OsSecretBackend("os", new()
    ///     {
    ///         Mount = os.Path,
    ///         MaxVersions = 10,
    ///         SshHostKeyTrustOnFirstUse = true,
    ///     });
    /// 
    /// });
    /// ```
    /// 
    /// ## Notes
    /// 
    /// * This resource requires Vault 2.0.0 or later.
    /// * The OS Secrets Engine plugin must be registered before the mount is enabled. Use `vault.Plugin` to manage catalog registration when appropriate.
    /// * Use `vault.Mount` to create, tune, or remove the OS Secrets Engine mount.
    /// * When `SshHostKeyTrustOnFirstUse` is enabled, the first connection to a host will automatically trust and store its SSH host key.
    /// 
    /// ## Import
    /// 
    /// OS Secret backend can be imported using the `Mount`, e.g.
    /// 
    /// ```sh
    /// $ pulumi import vault:index/osSecretBackend:OsSecretBackend os os
    /// ```
    /// </summary>
    [VaultResourceType("vault:index/osSecretBackend:OsSecretBackend")]
    public partial class OsSecretBackend : global::Pulumi.CustomResource
    {
        /// <summary>
        /// The maximum number of versions to keep. When omitted, Vault applies its server-side default of `10`. If you later remove the field from configuration, Vault retains the current value. Set to `0` to explicitly store zero in Vault.
        /// </summary>
        [Output("maxVersions")]
        public Output<int> MaxVersions { get; private set; } = null!;

        /// <summary>
        /// The path where the OS secrets engine is already mounted. Must not begin or end with a `/`.
        /// </summary>
        [Output("mount")]
        public Output<string> Mount { get; private set; } = null!;

        /// <summary>
        /// The namespace to provision the resource in.
        /// The value should not contain leading or trailing forward slashes.
        /// The `Namespace` is always relative to the provider's configured [namespace](https://www.terraform.io/docs/providers/vault/index.html#namespace).
        /// *Available only for Vault Enterprise*.
        /// </summary>
        [Output("namespace")]
        public Output<string?> Namespace { get; private set; } = null!;

        /// <summary>
        /// If `True`, SSH host keys will be trusted on first use (TOFU). If `False`, host keys must be explicitly configured. Defaults to `False`.
        /// </summary>
        [Output("sshHostKeyTrustOnFirstUse")]
        public Output<bool> SshHostKeyTrustOnFirstUse { get; private set; } = null!;


        /// <summary>
        /// Create a OsSecretBackend resource with the given unique name, arguments, and options.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resource</param>
        /// <param name="args">The arguments used to populate this resource's properties</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public OsSecretBackend(string name, OsSecretBackendArgs args, CustomResourceOptions? options = null)
            : base("vault:index/osSecretBackend:OsSecretBackend", name, args ?? new OsSecretBackendArgs(), MakeResourceOptions(options, ""))
        {
        }

        private OsSecretBackend(string name, Input<string> id, OsSecretBackendState? state = null, CustomResourceOptions? options = null)
            : base("vault:index/osSecretBackend:OsSecretBackend", name, state, MakeResourceOptions(options, id))
        {
        }

        private static CustomResourceOptions MakeResourceOptions(CustomResourceOptions? options, Input<string>? id)
        {
            var defaultOptions = new CustomResourceOptions
            {
                Version = Utilities.Version,
            };
            var merged = CustomResourceOptions.Merge(defaultOptions, options);
            // Override the ID if one was specified for consistency with other language SDKs.
            merged.Id = id ?? merged.Id;
            return merged;
        }
        /// <summary>
        /// Get an existing OsSecretBackend resource's state with the given name, ID, and optional extra
        /// properties used to qualify the lookup.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resulting resource.</param>
        /// <param name="id">The unique provider ID of the resource to lookup.</param>
        /// <param name="state">Any extra arguments used during the lookup.</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public static OsSecretBackend Get(string name, Input<string> id, OsSecretBackendState? state = null, CustomResourceOptions? options = null)
        {
            return new OsSecretBackend(name, id, state, options);
        }
    }

    public sealed class OsSecretBackendArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// The maximum number of versions to keep. When omitted, Vault applies its server-side default of `10`. If you later remove the field from configuration, Vault retains the current value. Set to `0` to explicitly store zero in Vault.
        /// </summary>
        [Input("maxVersions")]
        public Input<int>? MaxVersions { get; set; }

        /// <summary>
        /// The path where the OS secrets engine is already mounted. Must not begin or end with a `/`.
        /// </summary>
        [Input("mount", required: true)]
        public Input<string> Mount { get; set; } = null!;

        /// <summary>
        /// The namespace to provision the resource in.
        /// The value should not contain leading or trailing forward slashes.
        /// The `Namespace` is always relative to the provider's configured [namespace](https://www.terraform.io/docs/providers/vault/index.html#namespace).
        /// *Available only for Vault Enterprise*.
        /// </summary>
        [Input("namespace")]
        public Input<string>? Namespace { get; set; }

        /// <summary>
        /// If `True`, SSH host keys will be trusted on first use (TOFU). If `False`, host keys must be explicitly configured. Defaults to `False`.
        /// </summary>
        [Input("sshHostKeyTrustOnFirstUse")]
        public Input<bool>? SshHostKeyTrustOnFirstUse { get; set; }

        public OsSecretBackendArgs()
        {
        }
        public static new OsSecretBackendArgs Empty => new OsSecretBackendArgs();
    }

    public sealed class OsSecretBackendState : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// The maximum number of versions to keep. When omitted, Vault applies its server-side default of `10`. If you later remove the field from configuration, Vault retains the current value. Set to `0` to explicitly store zero in Vault.
        /// </summary>
        [Input("maxVersions")]
        public Input<int>? MaxVersions { get; set; }

        /// <summary>
        /// The path where the OS secrets engine is already mounted. Must not begin or end with a `/`.
        /// </summary>
        [Input("mount")]
        public Input<string>? Mount { get; set; }

        /// <summary>
        /// The namespace to provision the resource in.
        /// The value should not contain leading or trailing forward slashes.
        /// The `Namespace` is always relative to the provider's configured [namespace](https://www.terraform.io/docs/providers/vault/index.html#namespace).
        /// *Available only for Vault Enterprise*.
        /// </summary>
        [Input("namespace")]
        public Input<string>? Namespace { get; set; }

        /// <summary>
        /// If `True`, SSH host keys will be trusted on first use (TOFU). If `False`, host keys must be explicitly configured. Defaults to `False`.
        /// </summary>
        [Input("sshHostKeyTrustOnFirstUse")]
        public Input<bool>? SshHostKeyTrustOnFirstUse { get; set; }

        public OsSecretBackendState()
        {
        }
        public static new OsSecretBackendState Empty => new OsSecretBackendState();
    }
}
